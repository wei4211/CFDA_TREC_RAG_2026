...
async function embed(inputs: string[], env: Env): Promise<number[][]> {
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    ...
    try {
      ...
      if (attempt === 6)
        throw new Error(
          `embeddings fetch failed: ${e instanceof Error ? e.message : String(e)}`,
        );
      await sleep(Math.max(1000, 600 * 2 ** attempt));
      continue;
    }
    if (r.ok) {
    ...
    if (![429, 500, 502, 503, 504].includes(r.status) || attempt === 6)
      throw new Error(`embeddings HTTP ${r.status}`);
    await sleep(
      r.status === 429
        ? Math.min(30000, 2000 * 2 ** attempt)
        : Math.max(1000, 600 * 2 ** attempt),
    );
  }
...
export async function denseScores(
  ...
  env: Env,
  opts: { maxChars: number; batchSize: number } = {
    maxChars: 2000,
    batchSize: 24,
  },
  ...
  for (let i = 0; i < docs.length; i += opts.batchSize) {
    ...
    const vecs = await embed(
      batch.map((d) => d.text.slice(0, opts.maxChars)),
      env,
    );
    batch.forEach((d, j) => {
      if (vecs[j]) scores.set(d.docid, cosine(qVec, vecs[j]));
    });
    await sleep(150);
  }
  return scores;
