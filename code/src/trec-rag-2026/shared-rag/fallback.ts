...
export function buildExtractiveFallbackAnswerDraft(
  readDocs: Map<string, ReadDocument>,
): ExtractiveFallbackAnswerDraft {
  const selected = [...readDocs.values()]
    .filter((doc) => doc.text.trim().length > 0)
    .slice(0, 3);
  if (selected.length === 0)
    throw new Error(
      "Extractive answer fallback requires at least one read document with text.",
    );
  return {
...
function extractFallbackExcerpt(text: string): string {
  ...
  const candidate =
    sentences
      ...
      .find((sentence) => sentence.length >= 30) ?? normalized;
  const clipped =
    candidate.length <= 260
      ? candidate
      : `${candidate.slice(0, 257).trimEnd()}...`;
  return clipped.trim();
}
